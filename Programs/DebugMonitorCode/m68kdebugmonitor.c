...
int TestForSPITransmitDataComplete(void)    {
    int returnal = (SPI_Status & 0x80)>> 7;
    return returnal ;
...
void ProgramFlashChip(void)
{
    ...
    printf("\r\nLoading Program to SPI Flash.... wait a lil") ;
    RamPtr = 0x08000000;
    ...
    Enable_SPI_CS();
    garbage = WriteSPIChar(0x06);
    printf("\r\nPassed at %x. ", garbage);
    Disable_SPI_CS();
            // Erase command
    Enable_SPI_CS();
    garbage = WriteSPIChar(0xC7);
    printf("\r\nPassed at %x. ", garbage);
    Disable_SPI_CS();
            //query flash register
    Enable_SPI_CS();
    garbage = WriteSPIChar(0x05);
    while((WriteSPIChar(0x00) & 1)){printf("\r\nPassed at %x. ", (WriteSPIChar(0x00) & 1));}; //if lowest bit of flash's status register is 1: busy state so we wait here.
    printf("\r\nPassed at %x. ", garbage);
    Disable_SPI_CS();

    for (n = 0x000;n <= 0x3ff; n++){
            //Write enable command
        printf("\r\nPassed at %x. ", n);
        Enable_SPI_CS();
        garbage = WriteSPIChar(0x06);
        Disable_SPI_CS();
            //Write command
        Enable_SPI_CS();
        garbage = WriteSPIChar(0x02);
        garbage = WriteSPIChar((0xf00 & n)>>8);
        garbage = WriteSPIChar(0x0ff & n);
        garbage = WriteSPIChar(0x00);
        for (i=0;i<=255; i++){
            garbage = WriteSPIChar(*RamPtr);
            RamPtr++;
        }
        Disable_SPI_CS();
            //query flash register
        Enable_SPI_CS();
        garbage = WriteSPIChar(0x05);
        while((WriteSPIChar(0x00) & 1)){};//if lowest bit of flash's status register is 1: busy state so we wait here.
        Disable_SPI_CS();
    }
    main();
...
{
    signed int *RamPtr;
    unsigned int Mode, Start, End, i, num_patterns = 4, Eterat, PROG, PROG_leds, garbage;
    signed int Pat[];

/*get mode of operation*/
    printf("\r\nByte(0A), Word(0B), Longword(0C):") ;
    Mode = Get2HexDigits(0);
    for(i = 0; i < num_patterns; i++)
    {
	   if (Mode == 0x0A)
       {
            printf("\r\nByte Pattern: ");
            //c = tolower(_getch());
            //Pat[i] = (xtod(c) << 4) | (xtod(_getch()));
            Pat[i] = Get2HexDigits(0);
       }
       else if(Mode == 0x0B)
       {
            printf("\r\nWord Pattern: ");
            Pat[i] = Get4HexDigits(0);
       }
       else if(Mode == 0x0C)
       {
            printf("\r\nLongword Pattern: ");
            Pat[i] = Get8HexDigits(0);
       }
       else
       {
       printf("\r\nNo Mode ROMANOFzzzF:(");
...
/* get addresses - add your code to test memory here using 32 bit reads and writes of data between the start and end of memory*/
    printf("\r\nStart Address: ") ;
    Start = Get8HexDigits(0) ;
    printf("\r\nEnd Address: ") ;
    End = Get8HexDigits(0) ;

/* get bullshit addresses and abort*/
	if((Mode == 0x0C && (Start % 2 != 0)) || (Mode == 0x0C && (End % 2 != 0)) || (Mode == 0x0B && (Start % 2 != 0)) || (Mode == 0x0B && (End % 2 != 0)))
	{
	   if (Start % 2 != 0)
	   {
	   printf("\r\n Bullshit start address");
	   main();
	   }
	   if (End % 2 != 0)
	   {
	   printf("\r\n Bullshit end address");
	   main();
	   }
	return;
	}
...
/* If statement for Longword*/
	 if(Mode == 0x0C && Start % 2 == 0 && End % 2 == 0)
	 {
	       Eterat = (End - Start)/4; //number of iterations (E-ter-AAt)
	       for(i = 0; i < Eterat; i++)
	       {
	           PROG = (i + 1) * 100 / Eterat; //progress LEDS with bit-shift strategy
               PROG_leds = ((1 << (PROG/5)) - 1);
               PortA = (PROG_leds & 0xFF);
               PortB = (PROG_leds & 0xFF << 8) >> 8;
               PortC = (PROG_leds & 0x3 << 16) >> 16;
	           RamPtr = Start + (i * 4); //setting the address to write in
	           *RamPtr = Pat[i % 4] ; //setting the value of data
	           if(*RamPtr != Pat[i % 4])// checking to see if the write has been succesful
	               {
	                   printf("\r\nFailed at %x. ", RamPtr);
                       main();
	               }
...
/* If statement for Word*/
	 if(Mode == 0x0B && Start % 2 == 0 && End % 2 == 0)
	 {
	       Eterat = (End - Start)/2;//number of iterations (E-ter-AAt)
	       for(i = 0; i < Eterat; i++)
	       {
	           PROG = (i + 1) * 100 / Eterat;//progress LEDS with bit-shift strategy
               PROG_leds = ((1 << (PROG/5)) - 1);
               ...
               PortB = (PROG_leds & 0xFF << 8) >> 8;
               PortC = (PROG_leds & 0x3 << 16) >> 16;
	           RamPtr = Start + (i * 2);//setting the address to write in
               //RamPtr = Pat[i % 4] << 16;
	           if (i == Eterat - 1)//setting the value of data for the case of last iteration
	               {
	                   *RamPtr = (Pat[i % 4] << 16) | (*(RamPtr+2)>> 16);
	               }
	           else
	               {
	                   *RamPtr = Pat[i % 4] << 16; //setting the value of data for the rest of the iterations
	               }

	           if((*RamPtr != Pat[i % 4] << 16) && (i != Eterat - 1))// checking to see if the write has been succesful
	               {
	                   printf("\r\nFailed at %x. ", RamPtr);
                       main();
	               }
...
/* If statement for Byte*/
	 if(Mode == 0x0A)
	 {
	       ...
	       for(i = 0; i < Eterat; i++)
	       {
	           ...
	           if(i % 2 == 0 && i != Eterat - 1 ){
	           *RamPtr = (Pat[0] << 24) | (Pat[1] << 16);//setting the value of data for the rest of the even iterations
	           }
	           else if (i % 2 != 0 && i != Eterat - 1 )
	           {
	           *RamPtr = (Pat[2] << 24) | (Pat[3] << 16);//setting the value of data for the rest of the odd iterations
	           }
	           else if (i % 2 == 0 && i == Eterat - 1 )
	           {
	           *RamPtr = (Pat[0] << 24) | (Pat[1] << 16) | *(RamPtr+1)>> 16 ;//setting the value of data for the case of last iteration while Eterat is even
	           }
	           else if (i % 2 != 0 && i == Eterat - 1 )
	           {
	           *RamPtr = (Pat[2] << 24) | (Pat[3] << 16)| *(RamPtr+1)>> 16;//setting the value of data for the case of last iteration while Eterat is odd
	           }


	           if((*RamPtr == (Pat[0] << 24) | (Pat[1] << 16))||(*RamPtr == (Pat[2] << 24) | (Pat[3] << 16))) // checking to see if the write has been succesful
	               {
	                   printf("\r\nPassed at %x. ", RamPtr);
	               }
	           else
...
void main(void)
{
    ...
    char *BugMessage = "Aryan Chahardovalee:55476444 & Jonathan Zhang:40791428";
